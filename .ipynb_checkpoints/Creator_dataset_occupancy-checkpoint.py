...
class Creator_datset():

  def __init__(self, batchsize):
    self.batchsize = batchsize

  def create_dataset(self, direct):
    
    #directory = "02_created_data/" + dir + "/"
    directory = "02_created_data/piece/"
    path_csv = directory + direct + ".csv"
    ...
    print("Total\t" + str(len(file_paths)) + "\t" + str(len(dataset)) + " bt. x " + str(self.batchsize) + " bt.size" +  ": done")
...
if __name__ == "__main__":
    ...
    creator_dataset = Creator_datset(4)
    ds_train, ds_val, ds_test = creator_dataset.create_dataset("occupancy")
    ds_train, ds_val, ds_test = creator_dataset.create_dataset("piece")
